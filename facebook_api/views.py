    ...
    for i in range(30):
        try:
            ...
            campaign_reach = CampaignReach(
                ...
                oneday_view="".join(random.choices(string.digits, k=2)),
                oneday_click="".join(random.choices(string.digits, k=3)),
                cmpaign=campaign_details,
            )
            campaign_reach.save()
            campaign_value = CampaignValue(
                action_type="link_click",
                value=round(random.uniform(0.0, 0.9), 10),
                oneday_view_cost=round(random.uniform(10.0, 30.9), 10),
                oneday_click_cost=round(random.uniform(0.0, 0.9), 10),
                cmpaign=campaign_details,
            )
            campaign_value.save()
