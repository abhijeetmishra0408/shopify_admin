...
def getAccessScopes(request):
    ...
    payload = {}
    print(access_key)
    headers = {"X-Shopify-Access-Token": access_key, "content-type": "application/json"}
    print(headers)
    ...
    if r.status_code == 200:
        ...
        for product in r.json()["products"]:
            try:
                ...
                product_details = Products.objects.filter(
                    shopify_product_id=product["id"]
                ).update(
                    name=product["title"],
                    body_html=product["body_html"],
            ...
            for variants in product["variants"]:
                try:
                    product_variants = ProductsVariant(
                        ...
                        shopify_product_id=product["id"],
                        shopify_Variant_id=variants["id"],
                        price=variants["price"],
                        sku=variants["sku"],
                    )
                    product_variants.save()
                except:
                    product_variants = ProductsVariant.objects.filter(
                        shopify_Variant_id=variants["id"]
                    ).update(price=variants["price"], sku=variants["sku"])
...
def getHomePage(request):
    today = date.today()
    last_year = date(today.year -1, today.month + 1, 1)
    all_orders = Orders.objects.all()
    all_products = Products.objects.all()[0:10]
    chart_data = (
        Orders.objects.annotate(
            month=TruncMonth("order_date"), yearmonth=ExtractMonth("order_date")
        )
        .filter(order_date__gte=last_year)
        .values("month", "yearmonth")
        .annotate(counts=Count("id"))
        .values("yearmonth", "counts")
        .order_by("month")
    )
    product_data = (
        Orders.objects.values("product", "product__name")
        .annotate(total_price=Sum("price"))
        .order_by("-total_price")[0:10]
    )
    ...
    for each in chart_data:
        # print(each)
        orders_data["labels"].append(
            datetime.date(1900, each["yearmonth"], 1).strftime("%B")
        )
    ...
    return render(
        ...
        "index.html",
        {
            ...
            "sales": all_orders.aggregate(Sum("price")),
            "facebook_reach": facebook_campaign,
        },
    )
    ...
    for i in range(200):
        ...
        try:
            Orders.objects.create(
                shopify_order_id=random_product.shopify_Variant_id
                + i
                + random.choice(range_choice),
                currency="INR",
                price=random_product.price,
                status=random.choice(status_order),
                product=random_product.product,
                productvariant=random_product,
